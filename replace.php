...
$password = "";
$dbname = "food";
$sql = "SELECT SUM(it.price) AS total,om.name As Name FROM ordermain om,missing_items mi,items it where om.orderid=mi.orderid and om.idnum=$id and om.orderid=$orderid  and it.itemid=mi.itemid";
...
$_SESSION["total"]=	intval($totalrate);
...
<body>
    ...
    <h6 class="cusno"> Hi, <?php  echo $name;?> <br>
        Request Replace Order: <?php  echo $orderid;?><br>Email:  <?php  echo $email;?></h6>  <img class="logo" src="onlylogo.png" width="60px" height="50px"> </div>
...
letter-spacing: 0.096em;
   ...
   -webkit-filter: grayscale(100%);
    ...
    filter: gray;
		...
		<div style="margin-left:auto;margin-right:20px;margin-top:20px;">Missing order Rate Rs. : <?php echo $totalrate; ?></div>
...
$x = 0;
$btn="";			
if ($result->num_rows > 0 and $totalrate>0) {  
 while($row = $result->fetch_assoc()) { 
    
	 if(intval($row["available"])<=0){
		 
		echo"<div class='grayout'>" ;
		$btn = "N/A"; 
	 }
	 else{
     ...
     if($row["type"]=="veg"){
         echo'<div class="food '. $row["itemid"].'"><div class="vegfood"><div class="foodcontainer pd'. $row["itemid"].' veg">';
         ...
         echo'<div class="hrr"></div> <div class="theitem">
            ...
            <div class="itemvector '. $row["itemid"].'">' . $row["itemname"]. '</div>
                  ...
                  <div class="rightside">
                      ...
                      <button class="clickbfnadd pd'. $row["itemid"].' add" onclick="additem(this,\'cartshow\')" >
                      ...
                      <div class="addpinusbtn pd'. $row["itemid"].'">
                      <div class="addminusbtncontainer"><button class="minusbtn pd'. $row["itemid"].'" onclick="additem(this,\'minus\')">-</button><div class="no pd'. $row["itemid"].'">0</div><button  onclick="additem(this,\'add\')" class="plusbtn pd'. $row["itemid"].'">+</button></div>
...
document.getElementsByClassName("cart-table")[0].appendChild(para);
                   ...
                   function gotoselecteditem(){
                                    var elem = document.getElementsByClassName("foodcontainer pdItem2 veg")[0];
                elem.scrollIntoView();
                    }
                </script>
...
<div class="totalcontainer"><div class="totaltext">            <i class="fa fa-shopping-cart" onclick="opencart()" style="font-size:20px;cursor:pointer"></i>
      </div >
    
    
    
    <div class="threate">Rs. &nbsp<div class="totalrate">0</div></div></div>
    ...
    for (i = 0; i < nolist.length; i++) {
        ...
        if (parseInt(nolist[i].innerHTML) > 0) {
            if(parseInt(nolist[i].innerHTML)<21){
                  ...
                  idlist1[nolist[i].classList[1].slice(2)]=nolist[i].innerHTML; 
            }
 ...
 var idlists = JSON.stringify(idlist1);     
    ...
    form.setAttribute("action", "previeworderreplace.php");
    ...
    FN.setAttribute("value", "replace_order");       
        form.appendChild(FN);		 
...
function additem(obj,arg) {
   ...
   document.getElementsByClassName("no " + obj.classList[1])[0].innerHTML = (parseInt(document.getElementsByClassName("no " + obj.classList[1])[0].innerHTML) - 1).toString();}
                if(parseInt(document.getElementsByClassName("no " + obj.classList[1])[0].innerHTML)==0){
   document.getElementsByClassName("no " + obj.classList[1])[0].innerHTML = (parseInt(document.getElementsByClassName("no " + obj.classList[1])[0].innerHTML) - 1).toString();
            document.getElementsByClassName("addpinusbtn " + obj.classList[1])[0].style.display = "none";  
            document.getElementsByClassName("clickbfnadd " + obj.classList[1]+" add")[0].style.display = "block";          
...
document.getElementsByClassName("cart-table")[0].appendChild(para);
    ...
    document.getElementsByClassName("addpinusbtn " + obj.classList[1])[0].style.display = "none";
    document.getElementsByClassName("no " + obj.classList[1])[0].innerHTML = "0";
    }
    if(arg=="add"){
    if(parseInt(document.getElementsByClassName("no " + obj.classList[1])[0].innerHTML)<20){
